...
let how_much, startDate, endDate;
 
let aeps = [], astatus = [], status = [], num_watched_episodes = [], score = [], tags = [], priority = [], storage_type = [], storage_value = [], num_watched_times = [], rewatch_value = [], comments = [], is_asked_to_discuss = [], sns_post_type = [], is_rewatching = [], submitIt = [], checkIfCorrect =[];
            ...
            if (aeps.length == id.length) {
                ...
                for (f; f < id.length; f++) {
                    ...
                    try {
                        ...
                        let EPSlength = $(response).find('.spaceit_pad').length;
 
                        if (EPSlength == 0) {
                            checkIfCorrect.push("https://myanimelist.net/anime/"+id[f]);
                            let userID = dataLayer[0].userId;
                            $.ajax({
                                method: "POST",
